...
def check_plg():
    ...
    if int(xbmc.getInfoLabel("System.BuildVersion")[0:2]) > 14:
        ln = 1
    else:
        ln = 2
...
def update(name, dat, crash=None):
    payload = {'an': __scriptname__, 'av': __version__, 'ec': name, 'ea': 'tv_service', 'ev': '1',
               'dl': urllib.quote_plus(dat.encode('utf-8'))}
...
try:
    ...
    if check_plg():
        ...
        if len(sys.argv) > 1 and sys.argv[1] == 'False':
            ...
            xbmc.executebuiltin('AlarmClock (%s, RunScript(plugin.program.bscfusion, False), %s, silent)' % (
                __scriptid__, __addon__.getSetting('check_interval')))

        if b.gen_all(force):
            if __addon__.getSetting('en_cp') == 'true' and __addon__.getSetting('w_path') != '' and xbmcvfs.exists(
                    __r_path__):
                if os.path.isfile(os.path.join(__data__, '', 'bulsat.xml.gz')):
                    xbmcvfs.copy(os.path.join(__data__, '', 'bulsat.xml.gz'),
                                 os.path.join(__r_path__, '', 'bulsat.xml.gz'))
